...
int main(int argc, char **argv)
{
    ...
    for (;;)
    {
        ...
        case 1: // login业务
        {
            ...
            fixbug::ProxyRequest proxypequest;
            proxypequest.set_type(LOGIN);
            proxypequest.set_msg(loginrequest.SerializeAsString());

            g_isLoginSuccess = false;

           string request = proxypequest.SerializeAsString();
            ...
            if (g_isLoginSuccess);
            {
                // {
                    ...
                    g_currentUser.setName("mzy");

                    fixbug::GetFriendListRequest getfriendlistrequest;
                    getfriendlistrequest.set_userid(userid);

                    fixbug::ProxyRequest buffer;
                    buffer.set_type(GET_FRIENDLIST);
                    buffer.set_msg(getfriendlistrequest.SerializeAsString());
...

                {
                    ...
                    fixbug::ProxyRequest buffer;
                    buffer.set_type(GET_GROUPLIST);
                    buffer.set_msg(getgrouplistrequest.SerializeAsString());
                ...
                }
                {
                    ...
                    fixbug::ProxyRequest buffer;
                    buffer.set_type(GET_OFFLINEMSG);
                    buffer.set_msg(getofflinemsgrequest.SerializeAsString());
        ...
        case 2: // register业务
        {
            ...
            fixbug::ProxyRequest proxypequest;
            proxypequest.set_type(REG);
            proxypequest.set_msg(registerrequest.SerializeAsString());
...
void readTaskHandler(int clientfd)
{
    for (;;)
    {
        ...
        string buf(buffer);
        fixbug::ProxyRequest pbuf;
        pbuf.ParseFromString(buf);

        // 从字符流中读取前4个字节的内容
        uint32_t msgtype = pbuf.type();
        buf = pbuf.msg();
        ...
        if (GET_FRIENDLIST_ACK == msgtype)
        {
            ...
            if (getfriendlistresponse.result().success())
            {
                ...
                {
                    User user;
                    user.setId(getfriendlistresponse.friendlist().friend_(i).id());
                    user.setName(getfriendlistresponse.friendlist().friend_(i).name());
                    user.setState(getfriendlistresponse.friendlist().friend_(i).state());
        ...
        if (GET_GROUPLIST_ACK == msgtype)
        {
            ...
            if (getgrouplistresponse.result().success())
            {
                ...
                {
                    Group group;
                    group.setId(getgrouplistresponse.grouplist().group(i).id());
                    group.setName(getgrouplistresponse.grouplist().group(i).name());
                    group.setDesc(getgrouplistresponse.grouplist().group(i).desc());

                    int group_user_size = getgrouplistresponse.grouplist().group(i).users_size();

                    for (int j = 0; j < group_user_size; ++j)
                    {
                        User user;
                        user.setId(getgrouplistresponse.grouplist().group(i).users(j).id());
                        user.setName(getgrouplistresponse.grouplist().group(i).users(j).name());
                        user.setState(getgrouplistresponse.grouplist().group(i).users(j).state());
...
void addfriend(int clientfd, std::string friendid)
{
    ...
    fixbug::ProxyRequest buffer;
    buffer.set_type(ADD_FRIEND);
    buffer.set_msg(addfriendrequest.SerializeAsString());
...
void chat(int clientfd, std::string str)
{
    ...
    fixbug::ProxyRequest buffer;
    buffer.set_type(ONE_CHAT);
    buffer.set_msg(onechatrequest.SerializeAsString());
    string request = buffer.SerializeAsString();
...
void creategroup(int clientfd, std::string str)
{
    ...
    fixbug::ProxyRequest buffer;
    buffer.set_type(CREATE_GROUP);
    buffer.set_msg(creategrouprequest.SerializeAsString());
    string request = buffer.SerializeAsString();
...
void addgroup(int clientfd, std::string groupid)
{
    ...
    fixbug::ProxyRequest buffer;
    buffer.set_type(ADD_GROUP);
    buffer.set_msg(addgrouprequest.SerializeAsString());
    string request = buffer.SerializeAsString();
...
void groupchat(int clientfd, std::string str)
{
    ...
    fixbug::ProxyRequest buffer;
    buffer.set_type(GROUP_CHAT);
    buffer.set_msg(groupchatrequest.SerializeAsString());
    string request = buffer.SerializeAsString();
...
void loginout(int clientfd, std::string)
{
    ...
    fixbug::ProxyRequest buffer;
    buffer.set_type(LOGINOUT);
    buffer.set_msg(loginoutrequest.SerializeAsString());
    string request = buffer.SerializeAsString();
