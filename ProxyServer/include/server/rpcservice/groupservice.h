...
public:
                      ...
                      ::google::protobuf::Closure *done)
    {
        ...
        for (auto group_ : groupList)
        {
            ...
            grouptemp.set_id(group_.getId());
            grouptemp.set_name(group_.getName());
            grouptemp.set_desc(group_.getDesc());
            for (auto &member : group_.getUsers())
            {
                fixbug::User usertemp;
                
                usertemp.set_id(member.getId());
                usertemp.set_name(member.getName());
                usertemp.set_state(member.getState());
               // *grouptemp.add_users = usertemp;
